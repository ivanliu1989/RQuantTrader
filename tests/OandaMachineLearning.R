...
AUDUSD = prepareForexOandaPrices(oanda.count = 2500, Cur1 = "AUD", Cur2 = "USD", oanda.granularity = 'D')
aud = AUDUSD$OA.MID
aud.ret = log10(Cl(AUDUSD$OA.MID)/Op(AUDUSD$OA.MID))
aud.spreads = Hi(AUDUSD$OA.MID) - Lo(AUDUSD$OA.MID)

# 1. Generating features
main = finIndicatorHLC(HLC(aud))
colnames(aud.ret) = 'Returns'
main = merge(main, aud.ret)
main$Returns = lag(main$Returns, k = -1)
main = na.omit(main)


# 2 Model
# trainBC, testBC = NULL, predictors, response, reg = FALSE
main$Returns = ifelse(main$Returns > 0, 1, 0)
predictors = names(main)[!names(main) %in% c('Returns')]
response = 'Returns'
tot.n = nrow(main)
idx = round(0.75 * tot.n)
trainBC = as.data.frame(head(main, idx))
testBC = as.data.frame(tail(main, tot.n - idx))
trainBC$Returns = as.factor(trainBC$Returns)
testBC$Returns = as.factor(testBC$Returns)
reg = FALSE
