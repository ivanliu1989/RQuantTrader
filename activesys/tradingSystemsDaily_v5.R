...
configs = list(
    ...
    ,threshold = 0.3 # mean reversion threshold
    ,mean.threshold = 0.65 # mean reversion p value
    ...
    ,pred.n = 1 # Machine learning look ahead
    ...
    tryCatch({
        ...
        if(latestPrices$update.date > lastDate){
            ...
            if(mean.reversion.signal <= configs$mean.threshold & abs(zScore) >= configs$threshold){
                if(((zScore >= configs$threshold & aud.ml.ret <= configs$classifier.threshold) | aud.ml.ret <= 0.35) & abs(tail(vix$vix,1)) <= configs$VIX.threshold){ # Added VIX
                    goLong = FALSE
                    goShort = TRUE
                    closePos = FALSE
                }else if(((zScore <= -configs$threshold & aud.ml.ret >= configs$classifier.threshold) | aud.ml.ret >= 0.65) & abs(tail(vix$vix,1)) <= configs$VIX.threshold){ # Added VIX
                    goLong = TRUE
                    goShort = FALSE
                    closePos = FALSE
            ...
            if(goLong & (bid.price - AUDUSD.exe <= configs$exe.pip)){
                ...
                aud.take.profit = round(as.numeric(bid.price + sign(POS.AUDUSD) * configs$take.profit.multiplier * 2 * AUD.sd), 5)
                aud.stop.loss = round(as.numeric(bid.price - sign(POS.AUDUSD) * 2 * AUD.sd), 5)
                ...
                aud.take.profit = round(as.numeric(ask.price + sign(POS.AUDUSD) * configs$take.profit.multiplier * 2 * AUD.sd), 5)
                aud.stop.loss = round(as.numeric(ask.price - sign(POS.AUDUSD) * 2 * AUD.sd), 5)
