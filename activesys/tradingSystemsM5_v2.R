...
getLatestPrices = function(){
    AUDUSD = prepareForexOandaPrices(oanda.count = 2, Cur1 = "AUD", Cur2 = "USD", oanda.granularity = 'M5')
    USDCAD = prepareForexOandaPrices(oanda.count = 2, Cur1 = "USD", Cur2 = "CAD", oanda.granularity = 'M5')
...
AUDUSD = prepareForexOandaPrices(oanda.count = 2500, Cur1 = "AUD", Cur2 = "USD", oanda.granularity = 'M5')
USDCAD = prepareForexOandaPrices(oanda.count = 2500, Cur1 = "USD", Cur2 = "CAD", oanda.granularity = 'M5')
...
while(TRUE){
    tryCatch({
        ...
        if(latestPrices$update.date > lastDate){
            ...
            png(paste0("~/Common/audcad_intraday_m5/plots/audcad_intraday_m5_",format(index(tail(tmp.prices,1)), "%Y%m%d%H%M"),".png"))
            par(mfcol = c(3,1))
            print(chart_Series(zscores(spreads)))
            print(chart_Series(Z.Score))
            print(chart_Series(zscores.ma(spreads, look.back, round(look.back/6))))
            dev.off()
            ...
            updateDirFilePermissions('~/Common/audcad_intraday_m5/')
        }
    },
